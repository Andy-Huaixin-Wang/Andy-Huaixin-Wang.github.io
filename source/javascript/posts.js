...
function factory(data) {
    var search = decodeURI(location.search.substr(1)).toLowerCase().replace("/", "");
    ...
    if (pathname == "/note/open") {
        title.text("NOTE");
        isNote = true;
        dataList = data.note;
    }
    else if (pathname == "/temporary/open") {
        title.text("TEMPORARY");
        isTemporary = true;
        dataList = data.temporary;
    }
    else {
        dataList = data;
    }
        ...
        $.ajax({
            ...
            success: (data => {
                if (postItem.type == "markdown") {
                    ...
                    var headEnd = headStart + data.substr(headStart).indexOf("</head>");
                    var bodyStart = headEnd + data.substr(headEnd).indexOf("<body>") + 6;
                    var bodyEnd = bodyStart + data.substr(bodyStart).lastIndexOf("</body>");

                    var headContent = data.substr(headStart, headEnd - headStart);
                    var bodyContent = data.substr(bodyStart, bodyEnd - bodyStart);
...
function downloadButton(href) {
    ...
    var downloadImg = $("<img>");
    downloadImg.attr("src", "/source/images/download.svg");
...
function backButton() {
    ...
    var backImg = $("<img>");
    backImg.attr("src", "/source/images/back.svg");
...
function toc(contentElement, tocButtonElement, tocListElement, tocContentElement) {
    ...
    contentElement.find("h1,h2,h3,h4,h5,h6").each(function (index, item) {
        ...
        var tageNumber = tageName.substr(1);
